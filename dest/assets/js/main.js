...
//#region ResumeParsing.Init

ResumeParsing.Init = function () {
  let initialWidth = $(window).width();
  const adjustResumeZoom = function (sw) {
    // Base body Font Size
    let bf = 14;
    const resumeItems = $(ResumeParsing.DOM.resume + " > div"),
  ...
  $(window).resize(function () {
    const newSw = $(window).width();
    if (newSw !== initialWidth) {
      adjustResumeZoom(newSw);
      initialWidth = newSw;
    }
  });

  adjustResumeZoom(initialWidth);
};
